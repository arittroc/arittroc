...
function ExpertiseTile({ area }: { area: ExpertiseArea }) {
  ...
  return (
    ...
    >
      <span className="w-11 h-11 rounded-2xl glass flex items-center justify-center text-primary mb-5">
        <Icon className="w-5 h-5" />
      </span>
      <h3 className="text-lg font-bold tracking-tight mb-2">{area.title}</h3>
...
export default function ExperienceTimeline() {
  return (
    <section id="experience" className="py-24 px-6 max-w-7xl mx-auto scroll-mt-28">
      <div className="mb-14 max-w-2xl">
        <p className="font-mono text-xs uppercase tracking-[0.3em] text-accent mb-4">
          Track record
        </p>
        <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">
          Five Years, Full Stack to Bare Metal
        </h2>
      ...
      >
        ...
        >
          <div className="relative">
            ...
            <ol className="relative grid grid-cols-3 md:grid-cols-6 gap-y-8">
              {MILESTONES.map((m) => (
