...
/**
 * "Book a Demo" overlay. Submission POSTs the payload to the n8n webhook
 * (VITE_DEMO_WEBHOOK_URL) which appends a row to the target Google Sheet
 * and fires the alert workflow. Without the env var (local dev) it
 * simulates success so the UX remains testable.
 */
...
const inputClasses =
  'w-full glass rounded-2xl px-4 py-3 text-sm outline-none placeholder:text-muted-foreground/50 ' +
...
async function submitDemoRequest(payload: DemoRequest): Promise<void> {
  ...
  if (!webhook) {
    ...
    console.info('[DemoModal] VITE_DEMO_WEBHOOK_URL not set; payload:', payload);
    return;
  }
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg glass rounded-[2rem] p-8 md:p-10 max-h-[90vh] overflow-y-auto bg-white/75"
          >
            <button
              onClick={onClose}
              aria-label="Close"
            ...
            {status === 'success' ? (
              <div className="text-center py-10">
                ...
                <h3 className="text-2xl font-bold tracking-tight mb-3">Request received</h3>
                ...
                <button
                  onClick={onClose}
                  className="glass-button px-7 py-3 rounded-full text-sm font-semibold"
                >
                  Done
                </button>
              </div>
              ...
              <>
                ...
                <h3 id="demo-modal-title" className="text-3xl font-bold tracking-tight mb-2">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  {status === 'error' && (
                    <p className="flex items-center gap-2 text-sm text-red-600 bg-red-50/80 border border-red-100 rounded-xl px-4 py-3">
                  ...
                  <button
                    ...
                    className="glass-button mt-2 px-7 py-3.5 rounded-full font-semibold flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-wait"
                  >
                    {status === 'submitting' ? (
                      <>
