      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute top-full mt-4 left-1/2 -translate-x-1/2 glass rounded-2xl p-2 w-64 flex flex-col gap-1"
          >
            {CONNECT_ACTIONS.map((action) => (
              ...
              >
                <span className="w-9 h-9 rounded-xl glass flex items-center justify-center text-primary shrink-0">
...
export default function Navbar({ onBookDemo }: { onBookDemo: () => void }) {
  ...
  return (
    ...
    >
      <nav className="glass rounded-3xl md:rounded-full px-4 md:px-6 py-3 flex items-center justify-between gap-4">
        {/* Brand */}
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onBookDemo}
            className="glass-button px-4 md:px-5 py-2 rounded-full text-sm font-semibold flex items-center gap-2 group"
          >
            Book a Demo
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden glass rounded-3xl mt-2 p-4 flex flex-col gap-1"
          >
            {NAV_LINKS.map((link) => (
              <a
                key={link.href}
                ...
                className="px-3 py-2.5 rounded-xl font-medium hover:bg-white/70 transition-colors"
              >
                {link.label}
              </a>
            ))}
